...
func (r *ReconcileV2VVmware) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		...
		// TODO reconsider due to missing secret we can get infinit loop here
	...
	// TODO this should be optimized not to call getVMs everytime Reconcile is called
	err = readVmsList(r, request, connectionSecret, instance.Name)
	if err != nil {
		reqLogger.Error(err, "Failed to read list of VMWare VMs.")
		// Do not requeue when we failed to collect vms. Possibly we need user to update connection secret to reconcile again
	}
	return reconcile.Result{}, nil
}
